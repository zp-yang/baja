...
class EDHFilter(AbstractFilter):
    ...
    def update(
        ...
        # Define the flow step
        # P can be either (state_dim, state_dim) for EDH 
        # or (num_particles, state_dim, state_dim) for LEDH.
        is_localized = (P.ndim == 3)
        
        dlambda = 1.0 / self.flow_steps
        
        def flow_step(carry, lam):
            particles = carry
            
            # Recompute h(x) and H(x) at the current particle locations
            hx = jax.vmap(self.h)(particles)
            Hx = jax.vmap(jax.jacfwd(self.h))(particles)
            
            # Flow equation: dx/dlam = P * H^T * (R + lam * H * P * H^T)^{-1} * (y - h(x))
            def single_particle_flow(x_i, h_i, H_i, P_i):
                # Innovation
                innov = y - h_i
                
                # S_lam = R + lam * H_i * P_i * H_i^T
                S_lam = R + lam * (H_i @ P_i @ H_i.T)
                
                # Gain K_lam = P_i * H_i^T * S_lam^{-1}
                # To avoid inversion, solve S_lam * K_lam^T = H_i * P_i => K_lam = solve(S_lam, H_i @ P_i).T
                K_lam = jax.scipy.linalg.solve(S_lam.T, H_i @ P_i).T
                
                return K_lam @ innov

            if is_localized:
                dx_dlam = jax.vmap(single_particle_flow)(particles, hx, Hx, P)
            else:
                # Same P for all particles
                dx_dlam = jax.vmap(lambda x_i, h_i, H_i: single_particle_flow(x_i, h_i, H_i, P))(particles, hx, Hx)
                
            particles_next = particles + dlambda * dx_dlam
            return particles_next, None

        # lambdas = [0, dlambda, 2*dlambda, ..., 1 - dlambda]
        lambdas = jnp.linspace(0.0, 1.0 - dlambda, self.flow_steps)
        
        final_particles, _ = jax.lax.scan(flow_step, state.particles, lambdas)
        ...
        if return_likelihood:
            ...
            S = jax.jacfwd(self.h)(mean_pred) @ (P[0] if is_localized else P) @ jax.jacfwd(self.h)(mean_pred).T + R
            ll = jax.scipy.stats.multivariate_normal.logpdf(y, mean=z_pred, cov=S)
...
class LEDHKNNFilter(EDHFilter):
    ...
    def _compute_covariances(self, particles: jnp.ndarray) -> jnp.ndarray:
        ...
        dist_matrix = sq_norms[:, None] + sq_norms[None, :] - 2 * (particles @ particles.T)
...
class LEDHGMMFilter(EDHFilter):
    ...
    def _compute_covariances(self, particles: jnp.ndarray) -> jnp.ndarray:
        ...
        global_cov = (global_diff.T @ global_diff) / (N - 1.0) + jnp.eye(state_dim) * 1e-6
        
        def kmeans_step(mu, _):
            # Compute pairwise distances
            sq_norms_p = jnp.sum(particles**2, axis=1)
            sq_norms_m = jnp.sum(mu**2, axis=1)
            dist_matrix = sq_norms_p[:, None] + sq_norms_m[None, :] - 2 * (particles @ mu.T)
        ...
        def compute_cluster_cov(c):
            ...
            return jnp.where(count > 1, cov_sum / (count - 1.0) + jnp.eye(state_dim)*1e-2, global_cov)
            
        cluster_covs = jax.vmap(compute_cluster_cov)(jnp.arange(M)) # (M, state_dim, state_dim)
