...
class AbstractFilter(eqx.Module):
    ...
    def predict(
        self, state: State, u: Optional[jax.Array] = None, params: Any = None
    ) -> State:
        """
    ...
    def update(
        self, state: State, y: jax.Array, params: Any = None, return_likelihood: bool = False
    ) -> State:
        """
    ...
    def filter_sequence(
        ...
        def scan_step(state: GaussianState, inputs: Tuple[jax.Array, jax.Array, Any]):
            ...
            actual_params_t = params_t if (not isinstance(params_t, jax.Array) or params_t.size > 0) else None
    ...
    def smooth_sequence(
        ...
        def scan_step(next_smoothed_state: State, inputs: Tuple[State, jax.Array, Any]):
            ...
            actual_params_k = params_k if (not isinstance(params_k, jax.Array) or params_k.size > 0) else None
        ...
        Params_seq = jax.tree_util.tree_map(lambda x: x[1:], Params) if Params is not None else jnp.zeros((num_steps - 1, 0))
        ...
        full_smooth = jax.tree_util.tree_map(
            ...
            history_smooth_k, final_smoothed_state
        )
