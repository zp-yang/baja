...
class ParticleState(eqx.Module):
    ...
    def cov(self) -> jnp.ndarray:
        ...
        cov_components = self.weights[:, None, None] * (
            diff[:, :, None] * diff[:, None, :]
        )
...
class ParticleFilter(AbstractFilter):
    ...
    def predict(
        ...
        return ParticleState(particles=particles_pred, weights=state.weights, key=key)

    def _resample(
        self, particles: jnp.ndarray, weights: jnp.ndarray, key: jax.Array
    ...
    def update(
        ...
        z_pred = h_vmap(state.particles) # z_pred shape: (num_particles, meas_dim)

        # Calculate log likelihoods
        log_likelihoods = jax.scipy.stats.multivariate_normal.logpdf(z_pred - z, mean=jnp.zeros(R.shape[0]), cov=R)
        ...
        updated_state = ParticleState(
            particles=final_particles, weights=final_weights, key=key
        )
    ...
    def smooth_step(
        self,
        state: ParticleState,
        next_smoothed_state: ParticleState,
        u: Optional[jax.Array] = None,
        params: Any = None,
    ) -> ParticleState:
        """
